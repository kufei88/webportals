<template>
  <el-container v-loading="loading" class="post-article">
    <el-header class="header">
      <el-select v-model="article.c_id" placeholder="请选择文章栏目" style="width: 150px;">
        <el-option
  ...
  export default {
    mounted: function () {
      ...
      if (from != null && from != '' && from != undefined) {
        ...
        //   _this.$message({type: 'error', message: '页面加载失败!'})
        // })
        getRequest("/asp/articleDetail.asp?id=" + id).then(response => {
          _this.loading = false;
    ...
    methods: {
      ...
      saveBlog(state){
        ...
        var _this = this;
        _this.loading = true;
        var z_content = _this.$refs.md.d_render.replace(/\"/g,"\\\"");//转义，将双引号替换成\"插入数据库
        postRequest("/asp/articleAdd.asp", {
          id: _this.id?_this.id:-1,
          title: _this.article.title,
          mdContent: _this.article.mdContent.replace(/\\/g,"/"),
          content: z_content.replace(/\\/g,"/"),//转义，将斜杠替换成/插入数据库
          c_id: _this.article.c_id,
          state: state,
...
//            }
      ...
      imgAdd(pos, $file){
        ...
        uploadFileRequest("/asp/uploadImg.asp", formdata).then(resp=> {
          var url = resp.data;
          // if (json.status == 'success') {
//            _this.$refs.md.$imgUpdateByUrl(pos, json.msg)
            _this.$refs.md.$imglst2Url([[pos, url]]);
            _this.$refs.md.$imgUpdateByUrl(pos, url);
          // } else {
          // }
        }).catch(function(error) {
          _this.$message({type: 'error', message: '上传失败，请联系管理员'});
        });
      },
      imgDel(pos){
      ...
      getCategories(){
        let _this = this;
        getRequest("/asp/categoryList.asp").then(resp=> {
          _this.categories = resp.data.rows;
        });
      },
      handleClose(tag) {
