  ...
  export default{
    methods: {
      addNewCate(){
        this.loading = true;
        var _this = this;
        postRequest('/asp/categoryAdd.asp', {cateName: this.cateName}).then(resp=> {
      ...
      handleEdit(index, row){
        ...
        this.$prompt('请输入新名称', '编辑', {
          ...
          if (value == null || value.length == 0) {
            ...
            _this.loading = true;
            postRequest("/asp/categoryAdd.asp", {id: row.c_id, cateName: value}).then(resp=> {
              _this.$message({
                type: 'success',
                message: '更新成功！'
              });
      ...
      deleteCate(ids){
        ...
        this.loading = true;
        //删除
        postRequest("/asp/categoryDelete.asp", { ids : ids }).then(resp=> {
          if (resp.data == 'error') {
            _this.$message({
              type: 'error',
