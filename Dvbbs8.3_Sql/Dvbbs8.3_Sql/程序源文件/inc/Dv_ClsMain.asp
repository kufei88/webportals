...
Class Cls_Forum
	...
	Private Sub GetGroupSetting()
		...
		End If
		Rem 判断用户组是否定时关闭访问 2010-2-2 动网.小易 
	...
	Public Sub TyUserMsg(username,ChkNow)
		...
		ChkMsgTime= int(Dvbbs.forum_setting(115))			' 多久检查一次是否有新短信，单位：分钟，如果ChkNow参数为0则即时检查
		...
		Set TyRs=Dvbbs.Execute("Select sender,Title,Content,sendtime From Dv_Message where incept =',"&UserGroupID&",'")	'用户组
	...
	Public Sub ShowTopTable()
		...
		Else
			...
			If sendmsgnum>0 Then
				sysmenu = Replace(sysmenu,"{$TyMsgStr}"," 收件箱(<b style='color:red'>"&sendmsgnum&"</b>) ")
			Else
				sysmenu = Replace(sysmenu,"{$TyMsgStr}"," 收件箱(0) ")
			End If
		...
		Else
			sysmenu = Replace(sysmenu,"{$Ty_medal}","<a href='medal_index.asp'>荣誉勋章</a><br/>")
		End If
	...
	Public Sub Head_var(IsBoard,idepth,GetTitle,GetUrl)
		...
		If BoardID>0 Then
			'是否加载首页版块导航，当版块太多时加载会慢，建议关闭此功能 by 牛头
			Dim isindexmenu
			...
			Else
			    isindexmenu=""
			End If
			NavStr = " <a href="""&(Forum_Info(11)&"""" & isindexmenu &"   style=""cursor:hand;""><b>"&Forum_info(0)&"</b></a> → ")
		Else
	...
	'检查验证码是否正确 modifty by reoaiq at 091020
